# Cards included in this implementation:
# courtyard, pawn, secret chamber, great hall, shanty town, steward, nobles
# ^ None of these are attack cards.
# If we want an agent that can play attack cards, we will have to implement them ourselves.
#!/usr/bin/python
...
class PlusCardsIfNoActionsTest(unittest.TestCase):
    def test_cardsIfNoActions(self):
        ...
        self.assertEqual(hands[0].deckActions, ['draw'])
    ...
    def test_noCardsIfActions(self):
        ...
        hands = action.waysToPlayCard(start)
        self.assertEqual(len(hands), 1)
        self.assertEqual(hands[0].deckActions, [])
...
class DiscardForCashTest(unittest.TestCase):
    ...
    def test_oneTypeOfDiscardableCard(self):
        ...
        self.assertEqual(hands[0].hand, ['province'])
        self.assertEqual(hands[0].discarded, ['province'])
        self.assertEqual(hands[0].cashOffset, 1)

        # discard both provinces
        self.assertEqual(hands[1].hand, [])
        self.assertEqual(hands[1].discarded, ['province', 'province'])
        self.assertEqual(hands[1].cashOffset, 2)

        # discard nothing
        self.assertEqual(hands[2].hand, ['province', 'province'])
        self.assertEqual(hands[2].discarded, [])
        self.assertEqual(hands[2].cashOffset, 0)
...
class ChooseTest(unittest.TestCase):
    def test_choose2(self):
        ...
        # +1 card, +1 action
        self.assertEqual(hands[0].deckActions, ['draw'])
        self.assertEqual(hands[0].actions, 2)
        self.assertEqual(hands[0].buys, 1)
        self.assertEqual(hands[0].cashOffset, 0)

        # +1 card, +1 buy
        self.assertEqual(hands[1].deckActions, ['draw'])
        self.assertEqual(hands[1].actions, 1)
        self.assertEqual(hands[1].buys, 2)
        self.assertEqual(hands[1].cashOffset, 0)

        # +1 card, +$1
        self.assertEqual(hands[2].deckActions, ['draw'])
        self.assertEqual(hands[2].actions, 1)
        self.assertEqual(hands[2].buys, 1)
        self.assertEqual(hands[2].cashOffset, 1)

        # +1 action, +1 buy
        self.assertEqual(hands[3].deckActions, [])
        self.assertEqual(hands[3].actions, 2)
        self.assertEqual(hands[3].buys, 2)
        self.assertEqual(hands[3].cashOffset, 0)

        # +1 action, +$1
        self.assertEqual(hands[4].deckActions, [])
        self.assertEqual(hands[4].actions, 2)
        self.assertEqual(hands[4].buys, 1)
        self.assertEqual(hands[4].cashOffset, 1)

        # +1 buy, +$1
        self.assertEqual(hands[5].deckActions, [])
        self.assertEqual(hands[5].actions, 1)
        self.assertEqual(hands[5].buys, 2)
        self.assertEqual(hands[5].cashOffset, 1)
...
''' 
We will keep the original script unedited, but here is a new environment where we
can change which cards are incorporated into the environment. We should come up with a 
list of cards we want to have in here. 

We have to figure out how to make an action card with two actions play both actions sequentially

New plan, pick cards that just have you choose actions out of a set of possibilities.
'''

CHAPEL_ACTION = TrashCards(4)
...
CARDS = {
    ...
    'chapel': Card(cost=2, action=CHAPEL_ACTION),
    'pawn': Card(cost=2, action=PAWN_ACTION),
    'harem': Card(cost = 6, cash =2, victory = 2),
...
'''I don't really know what fittest so far is. What do these tuples represent? It's only for their
A.I so maybe it doesn't matter

'''
FITTEST_SO_FAR = {'province': (10, 1), 'copper': (2, 1), 'estate': (0, 0), 'pawn': (0, 0), 'shanty-town': (3, 1),
    'great-hall': (3, 0), 'duchy': (4, 0), 'steward': (4, 0), 'chapel': (1, 3), 'harem': (2, 2), 'secret-chamber': (3, 2),
